...
class Department:
    ...
    def execute_activities(self):
        ...
        for activity in self.activities:
            ...
            yield timestamp, activity.name, self.name, self.complexity, execution_time
...
def generate_combined_event_log(num_products, durations):
    ...
    departments = ['A', 'B', 'C']
    max_activities_per_department = 3
    activity_index = 1

    for duration_list in durations:
        for product_index in range(1, num_products + 1):
            prev_timestamp = start_time
            for duration in duration_list:
                timestamp = prev_timestamp + datetime.timedelta(seconds=duration)
                prev_timestamp = timestamp
                department = departments[(activity_index - 1) // max_activities_per_department]
                activity_name = f'{department}{activity_index}'
                combined_event_log.append((timestamp, f'P{product_index}', activity_name, duration))
                activity_index = (activity_index % max_activities_per_department) + 1
                if activity_index == 0:
                    activity_index = max_activities_per_department
